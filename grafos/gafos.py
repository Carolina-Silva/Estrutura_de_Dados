...
class Grafo:
    ...
    def arvoreGeradoraMinima_PRIM(self, orig):
        ...
        while(True):
            ...
            for i in range(self.__nro_vertices):
                if(pai[i] != -1):# achou vÃ©rtices jÃ¡ visitado
                    grau = len(gr.__arestas[i])
                    for j in range(grau):# percorre os vizinhos do vÃ©rtice visitado
                        if(pai[gr.__arestas[i][j]] == -1):# achou vÃ©rtice vizinho nÃ£o visitado
    ...
    def arvoreGeradoraMinima_Kruskal(self, orig):
        ...
        while(True):
            menorPeso = math.inf
            for i in range(self.__nro_vertices):# percorre todos os vÃ©rtices
                grau = len(gr.__arestas[i])
                for j in range(grau):# percorre os vizinhos do vÃ©rtice visitado
